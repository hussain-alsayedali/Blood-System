...
const fetchAllRequests = async () => {
  try {
    const response = await fetch(apiUrl + "/nurse/getAllRequestes");
    if (!response.ok) {
    ...
    <div className="container">
      ...
      <div className="requests-section">
        ...
        {showSections.receiving && (
          <ul className="list">
            {filterById(requests.receivingRequests, searchTerm).map(
              (request, index) => (
                <li key={index} className="list-item">
                  Recipient ID: {request.recipientId}, Served by:{" "}
                  {request.nurseId}, Status: {request.requestStatus}, Request
                  Date: {formatDate(request.requestDate)}, Last Update:{" "}
      ...
      <div className="requests-section">
        ...
        {showSections.donation && (
          <ul className="list">
            {filterById(requests.donationRequests, searchTerm).map(
              (request, index) => (
                <li key={index} className="list-item">
                  Donor ID: {request.donorId}, Handled by(ID): {request.nurseId}
                  , Status: {request.requestStatus}, Request Date:{" "}
                  {formatDate(request.requestDate)}, Last Update:{" "}
                  {formatDate(request.updateDate)}
                </li>
              )
            )}
      ...
      <div className="requests-section">
        ...
        {showSections.infection && (
          <ul className="list">
            {filterById(requests.infectionRequests, searchTerm).map(
              (request, index) => (
                <li key={index} className="list-item">
                  Patient ID: {request.donorId}, Handled by(ID):{" "}
                  {request.nurseId}, Infection Type: {request.diseaseCatalogId},
                  Request Date: {formatDate(request.requestDate)}, Last Update:{" "}
                  {formatDate(request.updateDate)}
                </li>
              )
            )}
